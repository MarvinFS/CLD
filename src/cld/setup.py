...
from cld.daemon import is_daemon_running
...
def _validate_plugin_root(plugin_root: Path) -> bool:
    ...
    if not plugin_root.is_dir():
        ...
        return False
    if not (plugin_root / "src" / "cld").exists():
        _print_warn("Plugin root missing src/cld; continuing anyway.")
    return True
...
def _check_platform_requirements() -> None:
    _print_info("Windows may prompt for microphone permissions.")
...
def _spawn_daemon(plugin_root: Path) -> bool:
    ...
    if _is_frozen():
        ...
        if main_mod and hasattr(main_mod, "__compiled__"):
            # Nuitka: get exe path from sys.argv[0]
    ...
    _print_warn("Run 'cld start' to retry.")
    return False
...
def run_setup(args: argparse.Namespace) -> int:
    ...
    _print_info("CLD setup starting.")
    ...
    _print_info("Use 'cld config' to customize settings.")
    return 0


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="CLD setup")
    parser.add_argument(
        "--skip-model-download",
        action="store_true",
